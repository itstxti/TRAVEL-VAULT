...
export default function App() {
  ...
  const [lightbox, setLightbox] = useState<{ p: Photo[]; i: number } | null>(
    null
  );
    ...
    debounce(async () => {
      ...
      try {
        for (const id of idsToDelete) {
          await pushDeleteDestination(id);
        }
        ...
        for (const id of idsToPush) {
          ...
          if (d) {
            await pushDestination(d, userId);
          }
        }
        ...
        console.error(
          'Could not sync with Supabase, will retry on the next change',
          e
        );
  ...
  useEffect(() => {
    if (ready) {
      migrateIfNeeded(dest, setMigration);
    }
  }, [ready]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('travel-vault-photo-ready', handler);
  }, []);
  ...
  const update = (d: Destination) => {
    setDest(x => {
      ...
      if (prev) {
        const removedPhotoIds = prev.photos
          .filter(p => !d.photos.some(np => np.id === p.id))
          .map(p => p.id);

        removedPhotoIds.forEach(id =>
          pushDeletePhoto(id).catch(e =>
            console.error('Could not delete photo from Supabase', e)
          )
        );
      }
  ...
  const gallery = useMemo(
    () => dest.find(d => d.id === galleryId) ?? null,
    [dest, galleryId]
  );

  const notes = useMemo(
    () => dest.find(d => d.id === notesId) ?? null,
    [dest, notesId]
  );
   ...
   * Legal pages
   *
   * Travel Vault doesn't use React Router, so the two legal pages are
   * rendered through local application state instead of adding another
   * dependency just for two static pages.
   */
  ...
  return (
    <>
      ...
      <header className="hero">
        <div>
          ...
          <p className="brand-sub">
            Keep track of where you want to go, what you've planned,
          ...
          <div className="backup-row">
            {migration === 'running' && (
              <span className="sync-status">
                Uploading your vault to the cloud…
              </span>
            )}
            ...
            {migration === 'error' && (
              <span className="sync-status sync-status-error">
                ...
                <button
                  className="mini-btn"
                  onClick={() => migrateIfNeeded(dest, setMigration)}
                >
                  Retry
                </button>
              </span>
        ...
        {tab !== 'stats' && (
          <div className="filter-row">
            <div className="filter-chips">
              {(['all', 'want_to_go', 'planned', 'visited'] as const).map(
                f => (
                  <button
                    key={f}
                    className={
            ...
            >
              ...
              <option value="want_to_go">
                {statusLabels.want_to_go}
              </option>
              <option value="planned">
                {statusLabels.planned}
              </option>
              <option value="visited">
                {statusLabels.visited}
              </option>
            </select>
          </div>
        )}
          ...
          <>
            {tab === 'list' && (
              <List
                ...
                openGallery={d => setGalleryId(d.id)}
                openNotes={d => setNotesId(d.id)}
              />
            )}
      ...
      {gallery && (
        <GalleryModal
          dest={gallery}
          close={() => setGalleryId(null)}
          update={update}
          openLightbox={(p, i) => setLightbox({ p, i })}
        />
      )}
