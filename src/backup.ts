...
const mimeFromExt: Record<string, string> = { jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', gif: 'image/gif', webp: 'image/webp' };

// Bundles destinations + full-resolution photos into a single .zip: a
// data.json manifest plus one image file per photo (not base64-in-JSON,
// which would bloat the export ~33% for no reason).
export async function exportData(destinations: Destination[]): Promise<Blob> {
  const zip = new JSZip();
    ...
    destinations.map(async d => ({
      ...
      photos: await Promise.all(
        d.photos.map(async p => {
          ...
          if (!match) return { id: p.id, caption: p.caption, file: null };
          const [, mime, base64] = match;
          const filename = `${p.id}.${extFromMime(mime)}`;
          photosFolder.file(filename, base64, { base64: true });
          return { id: p.id, caption: p.caption, file: filename };
        })
      ),
    }))
  );
  ...
  zip.file(
    'data.json',
    JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), destinations: manifest }, null, 2)
  );
        ...
        (d.photos || []).map(async (p: any) => {
          ...
          if (p.file) {
            ...
            if (entry) {
              ...
              dataUrl = `data:${mimeFromExt[ext] || 'image/jpeg'};base64,${base64}`;
            }
          }
          if (dataUrl) await putPhoto({ id: p.id, dataUrl, caption: p.caption });
          return { id: p.id, caption: p.caption, dataUrl };
        })
      );
      return { ...d, photos };
    })
